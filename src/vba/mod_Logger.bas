...
Public Sub LogEvt(stepTxt As String, level As eLvl, msg As String, _
    ...
    With logBuf.data
        .item(logBuf.used, 1) = logBuf.runID         ' RunID
        .item(logBuf.used, 2) = Now                  ' Timestamp
        .item(logBuf.used, 3) = Environ$("USERNAME") ' User
        .item(logBuf.used, 4) = stepTxt              ' Step
        .item(logBuf.used, 5) = Choose(level, "INFO", "DETAIL", "WARN", "ERROR") ' Level Text
        .item(logBuf.used, 6) = msg                  ' Message
        .item(logBuf.used, 7) = extra                ' Extra Info
    End With
End Sub
