...
' ----------- --------------- ----------------------------------------------
' 2025-05-08  Cline (AI)      - Fixed three critical bugs in ReorganizeColumns:
'                               1. Removed nested ColumnLetterFromNumber helper function
'                                  that was causing "Only comments may appear after End Sub/
'                                  End Function" compiler error
'                               2. Replaced Cut/Insert with Copy/Insert/Delete to
'                                  preserve cell comments (red triangles)
'                               3. Fixed index-shift calculation with proper "If curAbs >= destCol" 
'                                  check that works correctly regardless of whether source
'                                  column is left or right of target
'                             - Added clarifying comments to the algorithm steps
'                             - Reduced debug logging noise while preserving essential
'                               logging needed for diagnostics
'                             - Fixed formatting and indentation in helper functions
...
Public Function ApplyAll(tbl As ListObject, wsData As Worksheet) As Boolean
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Entered ApplyAll", "Table=" & IIf(tbl Is Nothing, "[Nothing]", tbl.Name)
    ...
    If tbl Is Nothing Or wsData Is Nothing Then
        ...
        ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "ERROR: Invalid arguments", "TableIsNothing=" & (tbl Is Nothing)
        Exit Function
    End If
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Starting formatting sequence", tbl.Name

    ' --- Call individual formatting routines ---
    LogEvt PROC_NAME, lgDETAIL, "Calling DeleteDuplicateColumns..."
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Calling DeleteDuplicateColumns"
    Call DeleteDuplicateColumns(tbl) ' Delete known duplicates first

    LogEvt PROC_NAME, lgDETAIL, "Calling ApplyNumberFormats..."
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Calling ApplyNumberFormats"
    Call ApplyNumberFormats(tbl)

    LogEvt PROC_NAME, lgDETAIL, "Calling FormatTableLook..."
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Calling FormatTableLook"
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Calling FormatCategoryColors"
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Check BEFORE Reorg", "Category Exists=" & ColumnExists(tbl, "Category")
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Check BEFORE Reorg", "Contact Exists=" & ColumnExists(tbl, "Contact")
    ' --- End logging ---

    LogEvt PROC_NAME, lgDETAIL, "Calling ReorganizeColumns..."
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Calling ReorganizeColumns"
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Check AFTER Reorg", "Category Exists=" & ColumnExists(tbl, "Category")
    ' Add logging for column indices using the new helper function
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Check AFTER Reorg", "Contact Index=" & GetColumnIndex(tbl, "Contact")
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Check AFTER Reorg", "DeviceName Index=" & GetColumnIndex(tbl, "DeviceName")
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Check AFTER Reorg", "Category Index=" & GetColumnIndex(tbl, "Category")
    ' --- End logging ---

    LogEvt PROC_NAME, lgDETAIL, "Calling SortDataTable..."
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Calling SortDataTable"
    Call SortDataTable(tbl)
    
    LogEvt PROC_NAME, lgDETAIL, "Calling CreateShortNamesAndComments..."
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Calling CreateShortNamesAndComments"
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Attempting final AutoFit"
    ' tbl.Range.Columns.AutoFit
    ' LogEvt PROC_NAME, lgDETAIL, "Final AutoFit completed."
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Final AutoFit completed"
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "ApplyAll completed successfully", tbl.Name
    Exit Function
...
ApplyAllError:
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "ERROR in ApplyAll", "Err=" & errNum & " - " & errDesc
...
Private Sub FormatTableLook(tbl As ListObject)
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Attempting AutoFit"
    tbl.Range.Columns.AutoFit
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "AutoFit completed"

    ' Set specific widths AFTER autofit
    On Error Resume Next ' Ignore errors if columns don't exist
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Attempting to set Applicant width"
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Set Applicant Width", IIf(Err.Number <> 0, "ERROR: " & Err.Description, 25)
    Err.Clear ' Clear potential error before next step

    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Attempting to set DeviceName width"
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Set DeviceName Width", IIf(Err.Number <> 0, "ERROR: " & Err.Description, 40)
    Err.Clear ' Clear potential error before next step

    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Attempting to set CompanyRecap width"
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Set CompanyRecap Width", IIf(Err.Number <> 0, "ERROR: " & Err.Description, 40)
    Err.Clear ' Clear potential error before next step

    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Attempting to set Category width"
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Set Category Width", IIf(Err.Number <> 0, "ERROR: " & Err.Description, 15)
...
Private Sub FormatCategoryColors(tblData As ListObject) ' Changed parameter name to tblData
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Entered FormatCategoryColors", tblData.Name
    ...
    If catCol Is Nothing Then
        ...
        ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Category column NOT FOUND. Skipping.", tblData.Name
    ...
    Else
        ...
        ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Category column FOUND. Proceeding.", tblData.Name
    End If
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "Category formatting applied", tblData.Name
    Exit Sub
...
FormatError:
    ...
    ' StandaloneDebug.DebugLog "mod_Format", PROC_NAME, "ERROR in FormatCategoryColors", "Err=" & Err.Number & " - " & Err.Description
...
Public Sub ReorganizeColumns( _
    ...
    ' Debug info only when DebugMode is True
    If DebugMode Then
    ...
    For tgtPos = LBound(targetOrder) To UBound(targetOrder)
        ...
        ' 1. Copy the source column (preserves all content including comments)
        lc.Range.EntireColumn.Copy
        
        ' 2. Insert the copy at the target position
        lo.Parent.Columns(destCol).Insert Shift:=xlShiftToRight
        Application.CutCopyMode = False

        ' 3. Calculate absolute sheet column position of the original
        Dim curAbs As Long
        curAbs = firstTableCol + curPos - 1    'absolute sheet column of the original

        ' 4. Critical fix: After insertion, columns at or right of insertion shift
        '    This adjustment ensures we delete the correct original column
        ...
        ' 5. Delete the original column after copying
        '    Note: Using Copy+Insert+Delete instead of Cut+Insert preserves comments
